...
BEGIN
    ...
    IF (v_column_mappings IS NULL) THEN
        RETURN ''Error: Could not generate column mappings for table '' || :BRONZE_TABLE_NAME || ''. Check if table exists and has non-audit columns.'';
    END IF;
    -- ################### MODIFICATION END ###################

    -- Truncate all records from table before loading data 
    EXECUTE IMMEDIATE ''TRUNCATE TABLE '' || :BRONZE_TABLE_NAME;
    
    -- Check if there are any files in the source folder
    EXECUTE IMMEDIATE ''LIST @'' || v_stage_name || ''/'' || v_azure_file_path || '' PATTERN = '''''' || v_file_pattern || '''''''' ;
    ...
    v_sql_1 := ''COPY FILES INTO @'' || v_stage_name || ''/'' || v_archive_file_path || 
               '' FROM @'' || v_stage_name || ''/'' || v_azure_file_path || 
               '' PATTERN = '''''' || v_file_pattern || '''''''' ;
    EXECUTE IMMEDIATE v_sql_1;
    
    -- ################## MODIFICATION START ##################
    -- Dynamic SQL to copy files from stage to table using the dynamic mappings
    v_sql := ''COPY INTO '' || v_target_table_name || '' ('' || :v_target_column_list || '')'' ||
             '' FROM ('' ||
             ''   SELECT '' ||
             ''      '' || :v_column_mappings || '', '' ||
    ...
    INTO record_count, v_loaded_count, v_partial_count, v_failed_count, v_total_files
    FROM load_stats;
    ...
    RETURN :BRONZE_TABLE_NAME || '' | Total rows: '' || record_count || '' | Files loaded: '' || v_loaded_count || '' | Files partial: '' || v_partial_count || '' | Files failed: '' || v_failed_count || '' | Total files: '' || v_total_files || '' | Status: '' || v_load_status;

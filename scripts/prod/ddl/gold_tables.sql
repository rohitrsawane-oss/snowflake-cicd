...
--gold layer tables
create or replace dynamic table SCH_TRUSTCAB_GOLD.CITY_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH,
	MONTH_NAME,
	...
	BUCKET_SHARE
) target_lag = '10 days' refresh_mode = AUTO initialize = ON_CREATE warehouse = WH_DEV_DATA_ENGINEER
 as
WITH rtd AS (
  SELECT
      city_id,
...
(PARTITION BY city_id, TO_VARCHAR(month::DATE,'YYYY-MM')) AS total_repeat_in_month
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_REPEAT_TRIP_DISTRIBUTION_DYNAMIC
)
SELECT
    e.*,
...
FROM TRUSTCAB_DEV.SCH_TRUSTCAB_GOLD.EXECUTIVE_VIEW e
LEFT JOIN rtd r
  ON r.city_id = e.city_id
 AND r.month   = e.month
;

create or replace dynamic table SCH_TRUSTCAB_GOLD.EXECUTIVE_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH,
	MONTH_NAME,
...
WITH base AS (
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.FACT_TRIPS_DYNAMIC ft
  JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC  dd ON ft.date    = dd.date
  JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_CITY_DYNAMIC  dc ON ft.city_id = dc.city_id
),
agg AS (
  SELECT
      city_id,
...
pax AS (
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.FACT_PASSENGER_SUMMARY_DYNAMIC
  GROUP BY 1,2
),
tgt AS (
  SELECT
 ...
 COALESCE(t.city_id, n.city_id)                                          AS city_id,
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_TRIPS_DYNAMIC t
  FULL OUTER JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_NEW_PASSENGERS_DYNAMIC n
...
create or replace dynamic table SCH_TRUSTCAB_GOLD.PASSENGER_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH_KEY,
	MONTH_NAME,
...
WITH P AS (
 ...
 SUM(fps.REPEAT_PASSENGER)   AS REPEAT_PASSENGERS,
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.FACT_PASSENGER_SUMMARY_DYNAMIC fps
  JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_CITY_DYNAMIC dc ON dc.CITY_ID = fps.CITY_ID
  JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC dd ON dd.start_of_month   = fps.MONTH
  GROUP BY 1,2,3,4,5
),
RATINGS AS (
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.FACT_TRIPS_DYNAMIC ft
  JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC dd ON dd.start_of_month = ft.DATE
  GROUP BY 1,2
),
TARGETS 
AS (
  SELECT
    ...
    t.TOTAL_TARGET_TRIPS
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_NEW_PASSENGERS_DYNAMIC n
  LEFT JOIN TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_TRIPS_DYNAMIC t
    ON t.CITY_ID = n.CITY_ID AND t.MONTH = n.MONTH
),
RATING_TARGET AS (
  SELECT
  ...
  FROM TRUSTCAB_DEV.SCH_TRUSTCAB_SILVER.CITY_TARGET_PASSENGER_RATING_DYNAMIC
)
SELECT

...
create or replace dynamic table {DATABASE_NAME}.SCH_TRUSTCAB_GOLD.EXECUTIVE_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH,
	MONTH_NAME,
	START_OF_MONTH,
	TOTAL_TRIPS,
	TOTAL_REVENUE WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	TOTAL_DISTANCE_KM WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	AVG_PASSENGER_RATING,
	AVG_DRIVER_RATING,
	DISTANCE_KM_NEW,
	DISTANCE_KM_REPEATED,
	REVENUE_NEW WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	REVENUE_REPEATED WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	TRIPS_NEW,
	TRIPS_REPEATED,
	REVENUE_WEEKDAY WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	REVENUE_WEEKEND WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	TRIPS_WEEKDAY,
	TRIPS_WEEKEND,
	DAY_TYPE,
	NEW_PASSENGERS,
	...
	AVG_FARE_PER_KM_NEW WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	AVG_FARE_PER_KM_REPEAT WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO')
...
WITH base AS (
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.FACT_TRIPS_DYNAMIC ft
  JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC  dd ON ft.date    = dd.date
  JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_CITY_DYNAMIC  dc ON ft.city_id = dc.city_id
),
agg AS (
  SELECT
      city_id,
...
pax AS (
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.FACT_PASSENGER_SUMMARY_DYNAMIC
  GROUP BY 1,2
),
tgt AS (
  SELECT
 ...
 COALESCE(t.city_id, n.city_id)                                          AS city_id,
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_TRIPS_DYNAMIC t
  FULL OUTER JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_NEW_PASSENGERS_DYNAMIC n
...
create or replace dynamic table {DATABASE_NAME}.SCH_TRUSTCAB_GOLD.CITY_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH,
	MONTH_NAME,
...
(PARTITION BY city_id, TO_VARCHAR(month::DATE,'YYYY-MM')) AS total_repeat_in_month
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_REPEAT_TRIP_DISTRIBUTION_DYNAMIC
)
SELECT
    e.*,
...
FROM {DATABASE_NAME}.SCH_TRUSTCAB_GOLD.EXECUTIVE_VIEW e
LEFT JOIN rtd r
...
create or replace dynamic table {DATABASE_NAME}.SCH_TRUSTCAB_GOLD.PASSENGER_VIEW(
	CITY_ID,
	CITY_NAME,
	MONTH_KEY,
	MONTH_NAME,
	START_OF_MONTH,
	TOTAL_PASSENGERS WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	NEW_PASSENGERS,
	REPEAT_PASSENGERS,
	REPEAT_PASSENGER_RATE,
	AVG_PASSENGER_RATING WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	AVG_DRIVER_RATING WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	TARGET_AVG_PASSENGER_RATING WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	TARGET_NEW_PASSENGERS WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	NEW_PAX_TARGET_DELTA WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO'),
	NEW_PAX_TARGET_ATTAINMENT WITH TAG ({DATABASE_NAME}.SCH_TRUSTCAB_AUDIT.PII_INFO='NUMERIC_INFO')
) target_lag = '10 days' refresh_mode = AUTO initialize = ON_CREATE warehouse = WH_PROD_DATA_ENGINEER
 as
WITH P AS (
  SELECT
 ...
 SUM(fps.REPEAT_PASSENGER)   AS REPEAT_PASSENGERS,
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.FACT_PASSENGER_SUMMARY_DYNAMIC fps
  JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_CITY_DYNAMIC dc ON dc.CITY_ID = fps.CITY_ID
  JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC dd ON dd.start_of_month   = fps.MONTH
  GROUP BY 1,2,3,4,5
),
RATINGS AS (
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.FACT_TRIPS_DYNAMIC ft
  JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.DIM_DATE_DYNAMIC dd ON dd.start_of_month = ft.DATE
  GROUP BY 1,2
),
TARGETS 
AS (
  SELECT
    ...
    t.TOTAL_TARGET_TRIPS
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_NEW_PASSENGERS_DYNAMIC n
  LEFT JOIN {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.MONTHLY_TARGET_TRIPS_DYNAMIC t
    ON t.CITY_ID = n.CITY_ID AND t.MONTH = n.MONTH
),
RATING_TARGET AS (
  SELECT
  ...
  FROM {DATABASE_NAME}.SCH_TRUSTCAB_SILVER.CITY_TARGET_PASSENGER_RATING_DYNAMIC
)
SELECT
